...
class GitHubPublisher extends (_publisher || _load_publisher()).Publisher {
    ...
    getRelease() {
        ...
        return (0, (_bluebirdLstC || _load_bluebirdLstC()).coroutine)(function* () {
            return (0, (_restApiRequest || _load_restApiRequest()).githubRequest)(`/repos/${ _this3.info.owner }/${ _this3.info.repo }/releases/${ _this3._releasePromise.value().id }`, _this3.token);
        })();
    }
    //noinspection JSUnusedGlobalSymbols
    deleteRelease() {
        ...
        return (0, (_bluebirdLstC || _load_bluebirdLstC()).coroutine)(function* () {
            if (!_this4._releasePromise.isFulfilled()) {
                return;
            }
            const release = _this4._releasePromise.value();
            if (release == null) {
                return;
            }
            for (let i = 0; i < 3; i++) {
                try {
                    ...
                    if (e instanceof (_restApiRequest || _load_restApiRequest()).HttpError && (e.response.statusCode === 405 || e.response.statusCode === 502)) {
                        continue;
                    }
                    throw e;
                }
