...
let getResolvedPublishConfig = exports.getResolvedPublishConfig = (() => {
    ...
    return function getResolvedPublishConfig(_x3, _x4, _x5) {
...
class PlatformPackager {
    ...
    doPack(outDir, appOutDir, platformName, arch, platformSpecificBuildOptions) {
        ...
        return (0, (_bluebirdLstC || _load_bluebirdLstC()).coroutine)(function* () {
            ...
            const resourcesPath = _this.platform === (_metadata || _load_metadata()).Platform.MAC ? _path.join(appOutDir, "Electron.app", "Contents", "Resources") : _path.join(appOutDir, "resources");
            (0, (_log || _load_log()).log)(`Packaging for ${ platformName } ${ (_metadata || _load_metadata()).Arch[arch] } using electron ${ _this.info.electronVersion } to ${ _path.relative(_this.projectDir, appOutDir) }`);
            yield (0, (_dirPackager || _load_dirPackager()).pack)(_this, appOutDir, platformName, (_metadata || _load_metadata()).Arch[arch], _this.info.electronVersion, (0, (_bluebirdLstC || _load_bluebirdLstC()).coroutine)(function* () {
                ...
                const ignoreFiles = new Set([_path.resolve(appDir, outDir), _path.resolve(appDir, _this.buildResourcesDir)]);
                ...
                const patterns = _this.getFileMatchers("files", appDir, _path.join(resourcesPath, "app"), false, fileMatchOptions, platformSpecificBuildOptions);
                ...
                let promise;
                if (asarOptions == null) {
                    promise = (0, (_filter || _load_filter()).copyFiltered)(appDir, _path.join(resourcesPath, "app"), filter, _this.info.devMetadata.build.dereference || _this.platform === (_metadata || _load_metadata()).Platform.WINDOWS);
                } else {
                    const unpackPattern = _this.getFileMatchers("asarUnpack", appDir, _path.join(resourcesPath, "app"), false, fileMatchOptions, platformSpecificBuildOptions);
                ...
                const promises = [promise, (0, (_util || _load_util()).unlinkIfExists)(_path.join(resourcesPath, "default_app.asar")), (0, (_util || _load_util()).unlinkIfExists)(_path.join(appOutDir, "version"))];
    ...
    checkFileInPackage(resourcesDir, file, messagePrefix, isAsar) {
        ...
        return (0, (_bluebirdLstC || _load_bluebirdLstC()).coroutine)(function* () {
            const relativeFile = _path.relative(_this2.info.appDir, _path.resolve(_this2.info.appDir, file));
            if (isAsar) {
            ...
            if (pathParsed.dir.indexOf(".asar") !== -1) {
                ...
                const outStat = yield (0, (_util || _load_util()).statOrNull)(_path.join(resourcesDir, "app", relativeFile));
                if (outStat == null) {
    ...
    archiveApp(format, appOutDir, outFile) {
        const isMac = this.platform === (_metadata || _load_metadata()).Platform.MAC;
        return (0, (_archive || _load_archive()).archiveApp)(this.devMetadata.build.compression, format, outFile, isMac ? _path.join(appOutDir, `${ this.appInfo.productFilename }.app`) : appOutDir, isMac);
    }
    ...
    getDefaultIcon(ext) {
        ...
        return (0, (_bluebirdLstC || _load_bluebirdLstC()).coroutine)(function* () {
            ...
            if (resourceList.indexOf(name) !== -1) {
                return _path.join(_this4.buildResourcesDir, name);
            } else {
    ...
    getResource(custom) {
        ...
        return (0, (_bluebirdLstC || _load_bluebirdLstC()).coroutine)(function* () {
            if (custom === undefined) {
                ...
                return _path.resolve(_this5.projectDir, custom);
            }
            return null;
        })();
    }
