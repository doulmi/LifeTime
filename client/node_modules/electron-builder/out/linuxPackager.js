    ...
    packageInDistributableFormat(outDir, appOutDir, arch, targets) {
        ...
        return (0, (_bluebirdLstC || _load_bluebirdLstC()).coroutine)(function* () {
            ...
            for (let t of targets) {
                if (t instanceof (_platformPackager || _load_platformPackager()).TargetEx && !t.isAsyncSupported) {
                    yield t.build(appOutDir, arch);
                }
            }
            yield (_bluebirdLstC2 || _load_bluebirdLstC2()).default.map(targets, function (it) {
                const target = it.name;
                if (target === "zip" || target === "7z" || target.startsWith("tar.")) {
                    const destination = _path.join(outDir, _this2.generateName(target, arch, true));
                    return _this2.archiveApp(target, appOutDir, destination).then(function () {
                        return _this2.dispatchArtifactCreated(destination);
                    });
                } else {
                    return it instanceof (_platformPackager || _load_platformPackager()).TargetEx && it.isAsyncSupported ? it.build(appOutDir, arch) : null;
                }
            });
        })();
    }
