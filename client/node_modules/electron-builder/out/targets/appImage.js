...
class AppImageTarget extends (_platformPackager || _load_platformPackager()).TargetEx {
    ...
    build(appOutDir, arch) {
        ...
        return (0, (_bluebirdLstC || _load_bluebirdLstC()).coroutine)(function* () {
            (0, (_log || _load_log()).log)(`Building AppImage`);
            const packager = _this.packager;
            // avoid spaces in the file name
            const image = _path.join(_this.outDir, packager.generateName("AppImage", arch, true));
            yield (0, (_util || _load_util()).unlinkIfExists)(image);
            const appImagePath = yield appImagePathPromise;
            const args = ["-joliet", "on", "-volid", "AppImage", "-dev", image, "-padding", "0", "-map", appOutDir, "/usr/bin", "-map", _path.join(__dirname, "..", "..", "templates", "linux", "AppRun.sh"), "/AppRun",
            ...
            yield (0, (_util || _load_util()).exec)(process.env.USE_SYSTEM_FPM === "true" || process.arch !== "x64" ? "xorriso" : _path.join(appImagePath, "xorriso"), args);
